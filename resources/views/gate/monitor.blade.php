...
<body class="bg-slate-950 text-white min-h-screen flex flex-col justify-between p-8 select-none">
    ...
    <div class="bg-slate-900/50 p-6 rounded-3xl border border-slate-800">
        ...
        <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
            @forelse($latestLogs as $log)
                <div class="p-3 rounded-xl border {{ $log->status == 'DIIZINKAN' ? 'bg-emerald-500/5 border-emerald-500/20' : 'bg-rose-500/5 border-rose-500/20' }}">
                    <div class="flex justify-between items-center mb-1">
                        <span class="text-[10px] font-mono text-slate-500">{{ $log->created_at->format('H:i:s') }}</span>
                        <span class="text-[9px] px-2 py-0.5 rounded-full font-black {{ $log->status == 'DIIZINKAN' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-rose-500/20 text-rose-400' }}">{{ $log->status }}</span>
                    </div>
    ...
    <script>
        ...
        // Paksa kursor selalu fokus ke input scanner sehabis klik dimanapun
